...
<div class="max-w-2xl mx-auto py-6 sm:px-6 lg:px-8">
    <h1 class="text-2xl font-semibold mb-6">Edit Playlist</h1>
    <div class="bg-white shadow-md rounded-lg p-6">
        <form action="{{ route('playlists.update', $playlist->id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" id="playlistName" name="playlistName" value="{{ old('playlistName', $playlist->name) }}" required class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
            </div>
            ...
            <div class="mb-4">
                ...
                <select id="playlistSongs" name="playlistSongs[]" multiple class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                    @foreach($songs as $song)
                        <option value="{{ $song->id }}" {{ in_array($song->id, $playlist->songs->pluck('id')->toArray()) ? 'selected' : '' }}>
                            {{ $song->name }}
                        </option>
                    @endforeach
                </select>
